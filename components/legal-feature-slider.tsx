...
type LegalFeatureSliderProps = {
  features: LegalFeature[];
};

export function LegalFeatureSlider({ features }: LegalFeatureSliderProps) {
  ...
  const Legalfeature = features[activeIndex];

  return (
    <div className="relative mx-auto max-w-[1237px] px-6 pb-10">
      <button
        aria-label="Slide left"
        className="absolute left-2 top-1/2 z-10 hidden h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-[#151a5b] text-white md:flex"
        type="button"
        onClick={goPrev}
      >
        <svg
      ...
      >
        <div className="mx-auto flex max-w-[640px] flex-col items-center gap-3">
          ...
          <p className="text-sm text-[#7a7a7a]">{Legalfeature.desc}</p>
        </div>
      </div>
