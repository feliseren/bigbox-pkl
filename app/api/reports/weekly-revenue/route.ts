...
export async function GET() {
  ...
  const tableRows = Array.from(tableMap.values()).sort((a, b) => {
    if (a.date === b.date) {
  ...
  const rowHeight = 20;
  const columns = [
    { title: "No", width: 32, align: "center" },
    { title: "Tanggal", width: 110, align: "left" },
    { title: "Nama Produk", width: 180, align: "left" },
    { title: "Banyak Terjual", width: 90, align: "right" },
    { title: "Total Penjualan", width: 120, align: "right" },
  ] as const;
  const tableWidth = columns.reduce((sum, col) => sum + col.width, 0);
  const startX = margin;
  const maxRows = Math.max(0, Math.floor((tableTop - 120) / rowHeight) - 1);
  const visibleRows = tableRows.slice(0, maxRows);
  ...
  const truncateToFit = (value: string, width: number) => {
    const maxChars = Math.max(1, Math.floor(width / 6));
  ...
  const headerY = tableTop - rowHeight + 6;
  columns.forEach((col, index) => {
    const x = colX[index] + 4;
    const text = col.title;
    contentParts.push(textAt(x, headerY, text, 10));
  });
  visibleRows.forEach((row, rowIndex) => {
    const y = tableTop - rowHeight * (rowIndex + 2) + 6;
    const cells = [
      String(rowIndex + 1),
      row.date,
      row.product,
    ...
    cells.forEach((cell, colIndex) => {
      ...
      if (col.align === "right") {
        x = colX[colIndex] + col.width - 4 - Math.max(0, text.length * 5);
      }
      contentParts.push(textAt(x, y, text, 10));
    });
  });
